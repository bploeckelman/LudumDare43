...
public class QuadTree {
    ...
    private int getIndex(QuadTreeable entity){
        ...
        boolean topQuadrant = entity.collisionBounds.y > CENTER.y;

        // Object Fits completely in the bottom
        boolean bottomQuadrant = entity.collisionBounds.y + entity.collisionBounds.height < CENTER.y;

        if (entity.collisionBounds.x + entity.collisionBounds.width < CENTER.x) {
            if (topQuadrant) index = 0;
            else if (bottomQuadrant) index = 2;
        } else if (entity.collisionBounds.x > CENTER.x){
